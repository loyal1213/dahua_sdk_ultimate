...
void CALLBACK
RealDataCallBackEx(LLONG
lRealHandle,
DWORD dwDataType, BYTE
*pBuffer,
DWORD dwBufSize, LLONG
param,
LDWORD dwUser
)
{
...
bOnline = FALSE;
pDeviceInfo->
strErrorCode = "Device Disconnected";
...
*   Purpose:��������������CLIENT_SetAutoReconnect��������
*   OutputParam:         LLONG              :lLoginID        // ��¼ID
*   OutputParam:         char*              :pchDVRIP        // ��½�豸IP
*   OutputParam:         LONG               :nDVRPort        // ��½�豸�˿�
*   OutputParam:         LDWORD   		    :dwUser           // �û��Զ����ֶ�
*   Return:              void
*   Created:	        %2016%:%05%:%16%  
*   Revision Record:    date:author:modify sth
**************************************************************************************/
void CALLBACK
HaveReConnectFunc(LLONG
lLoginID,
char *pchDVRIP, LONG
nDVRPort,
LDWORD dwUser
    ...
    while (it != g_DeviceList.end()) {
        ...
        if (pDeviceInfo->nBelongThread == nThreadID) {
            ...
            if (pDeviceInfo->lLoginHandle == 0) {
                ...
                if (CLIENT_QueryDevState(pDeviceInfo->lLoginHandle, DH_DEVSTATE_DEV_CHN_COUNT, (char *) &stuChn,
...
//           pDeviceInfo->nChannelCount = deviceInfo.nChanNum;
            ...
            for (int i = 0; i < nLoopCount; i++) {
                ...
                if (pDeviceInfo->channel[i].lRealPlayHandle != 0) {
                    CLIENT_SetRealDataCallBackEx2(pDeviceInfo->channel[i].lRealPlayHandle, RealDataCallBackEx,
                                                  (LDWORD) & (pDeviceInfo->channel[i]), 0x0f);
                } else {
