...
void Dialog::GetDiskInformation() {
    if (0 != m_lLoginHandle) {
        ...
        int nRetLen = 0;
        DWORD dwDiskNum = 0;
        BOOL bSuccess = CLIENT_QueryDevState(m_lLoginHandle, DH_DEVSTATE_DISK, (char *) &diskInfo,
                                             sizeof(DH_HARDDISK_STATE), &nRetLen);
        if (bSuccess && (nRetLen == sizeof(DH_HARDDISK_STATE))) {
            m_nCurDiskType = 0;
        } else {
            bSuccess = CLIENT_QueryDevState(m_lLoginHandle, DH_DEVSTATE_SD_CARD, (char *) &sdInfo,
                                            sizeof(DH_SDCARD_STATE), &nRetLen, 1000);
            if (bSuccess && (nRetLen == sizeof(DH_HARDDISK_STATE))) {
                m_nCurDiskType = 1;
            } else {
                m_nCurDiskType = 0;
...
void Dialog::on_ButBoot_2_clicked() {
    ...
    QString strVer = strTmpVer.left(1) + "." + strTmpVer.right(strTmpVer.length() - 1).left(2) + "." +
                     strTmpVer.right(strTmpVer.length() - 3);
